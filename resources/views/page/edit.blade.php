<x-main-layout>
    ...
    <x-form-section action="{{ route('page.edit', ['id' => $page->id]) }}">
        ...
        <x-form.checkbox name="private" checked="{{ $page->private }}"> Czy strona ma być prywatna? </x-form.checkbox>

        <x-jet-button type="submit" class="ml-2">Zapisz</x-jet-button>
    </x-form-section>

    <x-delete-item-button action="{{ route('page.delete', ['id' => $page->id, 'visibility' => $visibility]) }}"> </x-delete-item-button>

    <x-back-button action="{{ route($page->type . '.show', ['id' => $page->parent_id, 'visibility' => $visibility]) }}"></x-back-button>
